export const meta = {
  name: 'benefitclock-qa',
  description: 'Adversarial multi-dimension QA of the built BenefitClock site, with verification',
  phases: [
...
const CONTEXT = `
BenefitClock is a static, zero-dependency site (built by scripts/build.mjs) for U.S. seniors.
...
GOLDEN FACTS (authoritative — flag any contradiction):
  ...
  Canonical site domain is https://benefitclock.org (build injects {{SITE_URL}}). Any other domain (e.g. .com, www) in
...
const DIMENSIONS = [
  ...
  { key: 'factual-accuracy', effort: 'high', prompt: `DIMENSION: Factual accuracy & internal consistency. Grep every page for numbers, percentages, dates, and dollar amounts. Flag anything that contradicts the GOLDEN FACTS, any future-year figure not labeled an estimate, any inconsistent domain in JSON-LD (must be benefitclock.org / {{SITE_URL}}), any leftover {{TOKEN}} that didn't resolve in dist/, and any internally inconsistent statistic between pages.` },
